...
#this is the func for being next to heal water
func healWater():
	if Input.is_action_just_pressed("heal") and $"heal-water_bar".fill > 0 and $HEARTS.health < $HEARTS.max_health:
		$"heal-water_bar".empty()
		$HEARTS.heal_water()
	if heal_water == true:
		if Input.is_action_just_pressed("space"):
			$"heal-water_bar".fill()
			print ("Margaret never loved hasan")
